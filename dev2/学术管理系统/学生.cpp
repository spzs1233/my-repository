...
again:
	while (cin >> id) {
		if (id.length() == 9 && fileExists(id+".txt") == false) {
			ofstream file2("学生名单.txt", ios::app);
			file2<<id<<endl;
			file2.close();
			break;
		}
		else
			cout << "学号长度不正确或该学号已被注册，请重新输入" << endl;
	}
	id = id + ".txt";
...
int remove_student(string id) {
	...
	if (!existed) {
		cout<<"不存在该学生，无需注销"<<endl;
		return 0;
	}
	if (remove(id + ".txt") == 1) {
		cout << "学生 " << id << " 注销成功" << endl;
	}
	else {
		cout << "注销学生 " << id << " 失败" << endl;
	}
	return 0;
}
...
int student_menu(string id) {
	...
	while (out_of_system) {
		...
		case 2:
			remove_student(id);
			removeLinesFromFile("学生名单.txt", id);
			break;
		case 3:
